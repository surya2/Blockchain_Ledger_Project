...
def processTransfer(transfer_file, src_wallet, dest_tag, amount_to_transfer):
    ...
    with open(transfer_file, 'w') as file:
        ...
        # file.write("From: {0}\n".format(src_tag))
        # file.write("To: {0}\n".format(dest_tag))
        # file.write("Amount {0}\n".format(amount_to_transfer))
        # file.write("Date: {0}\n".format(time_of_transfer))
    # with open(transfer_file) as file:
    #     signed_hash = rsa.sign(file.read().encode(), privkeyBytes, 'SHA-256').hex()
    # with open(transfer_file, 'a') as file:
    #     file.write(signed_hash)
    print(
...
def getBalance(acct_tag):
    ...
    block_num = 1
    while os.path.isfile('./block_{0}.txt'.format(str(block_num))):
        with open('block_{0}.txt'.format(str(block_num))) as file:
...
def mine(leading_zeros):
    ...
    prev_filename = 'block_{0}.txt'.format(str(num_files-1))
    file_hash = hashFile(prev_filename)

    mempool = open('mempool/mempool.txt', 'r')
    mempool_lines = mempool.read().splitlines()

    new_block = 'block_{0}.txt'.format(str(num_files))
...
def validate():
    if not os.path.isfile("transaction_blocks/block_0.txt"):
        ...
        block = 1
        while os.path.isfile('./block_{0}.txt'.format(str(block))):
            cur_prevHash = hashFile('block_{0}.txt'.format(str(block - 1)))
            with open('block_{0}.txt'.format(str(block))) as file:
                lines = file.read().splitlines()
